...
from models_fleet import (
    _custom_endpoint_score,
    _load_custom_endpoint_stats,
    _load_model_live_stats,
    _model_score,
    _save_custom_endpoint_stats,
    _save_model_live_stats,
    _set_model_live,
    _set_MODEL_REGISTRY,
)
from prompts import (
...
def _resolve_sqlite_path(cfg: dict) -> Path:
    raw = (cfg.get("db_path") or cfg.get("path") or cfg.get("database") or "").strip()
    raw = _normalize_sqlite_path_input(raw)
    if not raw:
        raise ValueError("Podaj ścieżkę do pliku SQLite (.db) — np. /mnt/g/dane/eksport.db")
...
def _sql_fetch_column_sample(
    ...
    should_sample = ct in SQL_SAMPLE_VALUE_TYPES or "date" in ct or "time" in ct
    ...
    try:
        if dialect == "mssql":
            ...
            cur.execute(
                f"SELECT CAST({qcol} AS TEXT) AS v FROM {qtable} WHERE {qcol} IS NOT NULL LIMIT 1"
            )
        row = cur.fetchone()
...
def _build_sql_chart_config(columns: list[str], rows: list[dict]) -> dict | None:
    ...
    if (not label_col or not value_col) and len(columns) >= 2:
        if all(
            _is_numeric_sql_value(rows[i].get(columns[1], "")) for i in range(min(5, len(rows)))
        ):
...
app.config.update(
    ...
    PERMANENT_SESSION_LIFETIME=timedelta(
        seconds=int(os.environ.get("UI_SESSION_MAX_AGE", "43200"))
    ),
...
def _sanitize_upload_filename(filename: str) -> str:
    """Sanityzuje nazwę pliku uploadu — usuwa '../' i inne niebezpieczne sekwencje."""
...
OPENROUTER_MODEL_VERIFY = os.environ.get("OPENROUTER_MODEL_VERIFY", "google/gemini-2.5-flash")
OPENROUTER_FALLBACK_TO_OLLAMA = os.environ.get("OPENROUTER_FALLBACK_TO_OLLAMA", "true").lower() in (
    "1",
    "true",
    "yes",
)
...
).lower()  # openrouter (domyślnie dla LLM) | ollama (tylko dla importu/embeddings)
GROQ_MODEL = os.environ.get("GROQ_MODEL", "llama-3.3-70b-versatile").strip()

# Google Gemini (bezpośrednie API — audyt dużych plików / logów)
GEMINI_API_KEY = os.environ.get("GEMINI_API_KEY", "").strip()
# Gemini 2.0 Flash został wyłączony 2026-06-01; 2.5 Flash jest rekomendowanym następcą.
...
_env_gemini_model = os.environ.get("GEMINI_MODEL", GEMINI_MODEL_DEFAULT).strip()
GEMINI_MODEL = (
    GEMINI_MODEL_DEFAULT
    if _env_gemini_model in GEMINI_DEPRECATED_MODELS
    else (_env_gemini_model or GEMINI_MODEL_DEFAULT)
)
try:
    GEMINI_AUDIT_MAX_BYTES = max(
        1024 * 1024, int(os.environ.get("GEMINI_AUDIT_MAX_BYTES", str(12 * 1024 * 1024)))
    )
except ValueError:
...
try:
    OPENROUTER_AUDIT_MAX_CHARS = max(
        8000, int(os.environ.get("OPENROUTER_AUDIT_MAX_CHARS", "90000"))
    )
except ValueError:
...
try:
    GEMINI_RETRY_INITIAL_DELAY = max(
        0.5, float(os.environ.get("GEMINI_RETRY_INITIAL_DELAY", "2.5"))
    )
except ValueError:
...
def _get_latest_github_release() -> dict | None:
    ...
    if _github_release_cache["data"] and (
        now - _github_release_cache["timestamp"] < GITHUB_RELEASE_CACHE_SECONDS
    ):
    ...
    try:
        ...
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "AI-Analiza-Dokumentow",
            },
        )
...
def _require_api_key():
    ...
    if provided != APP_API_KEY:
        return jsonify(
            {
                "success": False,
                "error": "Brak autoryzowanej sesji UI lub nieprawidłowy nagłówek X-API-Key",
            }
        ), 401
    return None
...
def qdrant_switch():
    ...
    if mode == "local":
        ...
        if not QDRANT_CLOUD_URL:
            return jsonify(
                {"success": False, "error": "Brak konfiguracji Qdrant Cloud (QDRANT_CLOUD_URL)"}
            ), 400
        new_url, new_key = QDRANT_CLOUD_URL, QDRANT_CLOUD_KEY
    else:
        return jsonify(
            {"success": False, "error": "Nieprawidłowy tryb. Użyj 'local' lub 'cloud'"}
        ), 400
    with _qdrant_lock:
        QDRANT_URL = new_url
        QDRANT_KEY = new_key
    ...
    with _qdrant_lock:
        if _qdrant_client is None:
            ...
            _qdrant_client = QdrantClient(
                ...
                api_key=QDRANT_KEY,
                timeout=30.0,
                limits=Limits(max_keepalive_connections=5, max_connections=20, keepalive_expiry=30),
            )
...
def _init_embed_cache():
    ...
    try:
        with sqlite3.connect(_CACHE_DB_PATH, timeout=10) as conn:
            conn.execute(
                "CREATE TABLE IF NOT EXISTS embeddings (hash TEXT PRIMARY KEY, vector TEXT)"
            )
    ...
    try:
        ...
        if not client.collection_exists(ACTIVE_COLLECTION):
            client.create_collection(
                ACTIVE_COLLECTION, vectors_config=VectorParams(size=768, distance=Distance.COSINE)
            )
    ...
    for attempt in range(4):  # max 4 próby
        try:
            ...
            if "Connection reset by peer" in str(e) or "104" in str(e):
                ...
                logger.warning(
                    f"Ollama embedding reset (próba {attempt + 1}/4) — czekam {wait:.1f}s..."
                )
                time.sleep(wait)
                continue
            else:
...
def generate_answer(
    ...
    try:
        result = call_llm(
            prompt=prompt, system=system, stream=False, provider=provider, model=model
        )
        if isinstance(result, dict):
...
def verify_answer(
    answer: str, contexts: list, query: str, provider: str | None = None, model: str | None = None
) -> dict:
    ...
    prompt = VERIFY_PROMPT_TEMPLATE.format(contexts=context_str, query=query, answer=answer)
...
def _filter_documents_list(
    ...
    if modified_before:
        out = [
            d
            for d in out
            if _doc_modified_iso(d) <= modified_before[:10] or not _doc_modified_iso(d)
        ]
    return out
...
def _extract_xls(file_path: Path) -> str:
    ...
    try:
        ...
        for sheet in wb.sheets():
            ...
            hidden_info = []
            if hidden_rows:
                hidden_info.append(f"{len(hidden_rows)} ukrytych wierszy")
            if hidden_cols:
                hidden_info.append(f"{len(hidden_cols)} ukrytych kolumn")
            ...
            for ri in range(sheet.nrows):
                ...
                for ci in range(sheet.ncols):
                    ...
                    if ctype == xlrd.XL_CELL_DATE:
                        ...
                        ctype == xlrd.XL_CELL_NUMBER and float(val) == int(val) and abs(val) < 1e12
                    ):
                        val = int(val)
    ...
    for sheet_name in sheet_names:
        ...
        for ri_h, row in enumerate(source_rows[:5]):
            if sum(1 for c in row if c is not None and str(c).strip()) >= 2:
                headers = [
                    str(c).strip() if c is not None else f"Kol{i + 1}" for i, c in enumerate(row)
                ]
                header_row_idx = ri_h
                break
...
def extract_file_metadata(f_path: Path) -> dict:
    ...
    if ext == ".docx" and docx:
        try:
            ...
            if core_props.author:
                embedded["author"] = core_props.author
            if core_props.title:
                embedded["title"] = core_props.title
            if core_props.last_modified_by:
                embedded["last_modified_by"] = core_props.last_modified_by
            if core_props.created:
                embedded["created"] = str(core_props.created)
            if core_props.modified:
                embedded["modified"] = str(core_props.modified)
            if embedded:
    ...
    if ext in [".xlsx", ".xls"] and openpyxl:
        try:
            ...
            embedded = {}
            if props.creator:
                embedded["author"] = props.creator
            if props.title:
                embedded["title"] = props.title
            if props.lastModifiedBy:
                embedded["last_modified_by"] = props.lastModifiedBy
            if props.created:
                embedded["created"] = str(props.created)
            if props.modified:
                embedded["modified"] = str(props.modified)
            if embedded:
...
def browse():
    ...
    try:
            ...
            try:
                if child.is_dir():
                    ...
                    if show_all or is_doc:
                        if not _mtime_in_range(
                            child, modified_after or None, modified_before or None
                        ):
                            continue
                        try:
                            st = child.stat()
        ...
        has_hidden_or_other = (
            (total_children > 0) and (len(entries) == 0) and (permission_denied == 0)
        )
        ...
        while max_idle_iterations < 600:  # max 10 minut bez zmian (600 * 1s)
            try:
                ...
                changed = (
                    ...
                    or last_sent["done"] != task.done
                    or last_sent["status"] != task.status.value
                )
    ...
    try:
        ...
        for i, f_path in enumerate(files):
            ...
            try:
                ...
                # Deduplikacja
                chunk_ids = [
                    hashlib.md5(c.encode("utf-8", errors="replace")).hexdigest() for c in chunks
                ]
                existing_ids = set()
                ...
                new_chunks_data = [
                    (cid, chunk) for cid, chunk in zip(chunk_ids, chunks) if cid not in existing_ids
                ]
    ...
    if async_mode:
        try:
            ...
            if not folder_path.is_dir() or not _path_is_allowed(folder_path):
                return jsonify(
                    {"success": False, "error": "Ścieżka niedozwolona lub poza SEARCH_ROOTS"}
                ), 403
    ...
    def generate():
        ...
        try:
            ...
            if not files:
                yield sse(
                    "done",
                    {"count": 0, "chunks": 0, "skipped": 0, "msg": "Brak kompatybilnych plików."},
                )
                return

            ok, task = _start_heavy_task(
                "document_import",
                f"Import dokumentów: {folder_path}",
                {
                    "folder": str(folder_path),
                    "ext": list(exts),
                    "force_ocr": force_ocr,
            ...
            _update_heavy_task(
                task_id, done=len(files), total=len(files), progress_pct=100, current_item=None
            )
            yield sse(
                "done",
                {
...
def clear_collection():
    try:
        ...
        client.create_collection(
            ACTIVE_COLLECTION, vectors_config=VectorParams(size=vsize, distance=Distance.COSINE)
        )
        _suggestions_cache["data"] = None
        _docs_cache["data"] = None
        _COLLECTION_PROFILE_CACHE.clear()
...
def file_open():
    ...
    if not check_path.is_file() or not _path_is_allowed(check_path):
        return jsonify(
            {"success": False, "error": "Plik niedostępny lub poza dozwolonymi katalogami"}
        )
            ...
            try:
                for token in stream_llm_tokens(
                    ...
                    if isinstance(token, str) and (
                        token.startswith("[Błąd")
                        or token.startswith("[RATE_LIMIT]")
            ...
            return jsonify(
                {
                    "success": True,
                    "results": [],
                    "ai_answer": "Brak dokumentów dla wybranego pliku.",
                }
            )
...
def compare_documents():
    ...
    def generate():
        ...
        try:
            ...
            comparison_text = ""
            for token in stream_llm_tokens(
                prompt=prompt, system=system, max_tokens=2000, temperature=0.2
            ):
...
def collection_profile():
    ...
    try:
        ...
        if ext_counts.keys() & financial_exts and top_ratio > 0.5 and top_ext in financial_exts:
            ...
            suggestion_mode = "normal"
            suggestion_text = (
                f"Jednorodna kolekcja ({top_ext.upper()}). Tryb normalny jest odpowiedni."
            )
        else:
            profile = "mixed"
            suggestion_mode = "detective"
    ...
    with _SWARM_OR_SEMAPHORE:
        try:
            ...
            if (
                ...
                and not text.startswith("[RATE_LIMIT]")
            ):
                return text, model_name
...
def _prepare_swarm_context_chunks(contexts: list, max_chars_per_chunk: int) -> list[dict]:
    ...
    limit = max(
        SWARM_CHARS_PER_CHUNK_MIN, int(max_chars_per_chunk or SWARM_DEFAULT_MAX_CHARS_PER_CHUNK)
    )
    return [
        {
            "file": c.get("file", ""),
    ...
    def generate():
        try:
            ...
            def _run_worker(worker_id: int, context_chunks: list, model_id: str, model_name: str):
                try:
                    ...
                    return (
                        ...
                        None,
                        model_name,
                        _format_swarm_worker_error(str(exc)),
                    )
                ...
                try:
                    ...
                    for fut in completed_futures:
                        ...
                        if kind == "done":
                            ...
                            yield sse(
                                "worker_error",
                                {"worker_id": worker_id, "model": model_name, "error": err},
                            )
            ...
            if len(partial) < n_workers:
                yield sse(
                    "error",
                    {"error": f"Timeout — workery nie odpowiedziały w czasie {worker_timeout} s."},
                )
                return
...
def get_suggestions():
    ...
    if (
        not force
        and _suggestions_cache["data"]
        and (now - _suggestions_cache["ts"]) < SUGGESTIONS_TTL
    ):
    ...
    try:
        ...
        context = "\n\n".join([f"[{p['file']}]: {p['text'][:600]}" for p in sample])
        prompt = (
...
def scan_noise():
    try:
        ...
        return jsonify(
            {"success": True, "noise": noise, "total_chunks": sum(n["chunks"] for n in noise)}
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
...
def export_metadata_report_docx():
    ...
    try:
        ...
        doc.add_paragraph(
            "Poniżej znajdują się metadane systemowe oraz wbudowane dla wybranych dokumentów. "
        ...
        p = doc.add_paragraph()
        run = p.add_run(
            "Wygenerowano przez AI Analiza Dokumentów | Narzędzie wspomagające analizę śledczą"
        )
        run.font.size = Pt(8)
...
def export_docx():
    ...
    try:
        ...
        doc.add_paragraph("Wygenerowano przez AI Analiza Dokumentów | Llama3 + Qdrant Cloud").runs[
            0
        ].font.size = Pt(8)
...
def _network_ai_briefing(
    query: str, nodes: list, edges: list, stats: dict, provider=None, model=None
) -> str:
...
def build_network():
    ...
    def generate():
        ...
        try:
            ...
            batches = [
                all_contexts[i : i + BATCH_SIZE] for i in range(0, len(all_contexts), BATCH_SIZE)
            ]
    ...
    def generate():
        ...
        try:
            ...
            if not payloads:
                yield sse(
                    "done",
                    {"success": True, "timeline": [], "message": "Brak dokumentów do analizy."},
                )
                return
            ...
            for p in payloads:
                ...
                if date_pattern.search(text):
                    date_chunks.append({"file": p.get("file", "Nieznany"), "text": text[:600]})
            ...
            context_str = "\n\n".join([f"[{c['file']}]: {c['text']}" for c in date_chunks[:15]])
            ...
            if json_match:
                ...
                yield sse("done", {"success": True, "timeline": parsed_timeline})
            else:
                logger.warning(f"Timeline: brak JSON w odpowiedzi LLM: {raw_resp[:200]}")
                yield sse(
                    "error",
                    {"error": "LLM nie zwrócił poprawnego formatu JSON.", "raw": raw_resp[:300]},
                )
...
def _excel_forensics(file_path: Path) -> dict:
    ...
    for sn in wb_v.sheetnames:
        ...
        hidden_cols = sorted(cl for cl, cd in ws.column_dimensions.items() if cd.hidden)
        ...
        if hidden_rows:
            ...
            _excel_add_finding(
                ...
                detail=(
                    ...
                    + (
                        "Próbka danych: " + "; ".join(samples[:4])
                        if samples
                        else "Wiersze bez danych liczbowych/tekstowych."
                    )
                ),
    ...
    findings.sort(
        key=lambda x: (sev_order.get(x.get("severity"), 3), x.get("sheet", ""), x.get("cell", ""))
    )
...
def analyze_excel():
    ...
    if not path:
        return jsonify(
            {
                "success": False,
                "error": f"Nie znaleziono pliku '{fname}' — zaimportuj go ponownie z opcją śledzenia ścieżek (full_path).",
            }
        )

    if openpyxl is None:
        return jsonify(
            {"success": False, "error": "Brak biblioteki openpyxl — zainstaluj zależności Python."}
        )
    ...
    if not path.is_file() or not _path_is_allowed(path):
        return jsonify(
            {
                "success": False,
                "error": "Plik niedostępny lub poza dozwolonymi katalogami (SEARCH_ROOTS)",
            }
        )
    ...
    notable = [f for f in result.get("findings", []) if f.get("severity") in ("critical", "high")]
...
def _sql_text_columns(conn, table: str, dialect: str) -> list[str]:
    ...
    try:
        if dialect == "sqlite":
            ...
            return [
                r[1] for r in cur.fetchall() if any(t in (r[2] or "").upper() for t in text_types)
            ]
        cur.execute(
            """
...
def sql_test():
    ...
    try:
        ...
        conn.close()
        return jsonify(
            {"success": True, "version": version[:120], "tables": tables, "dialect": dialect}
        )
    except Exception as e:
        return jsonify({"success": False, "error": str(e)})
...
def sql_ask():
    ...
    try:
        ...
        preview = "\n".join(
            [", ".join([f"{k}={v}" for k, v in r.items()]) for r in result_rows[:10]]
        )
        prompt_interp = (
...
def sql_write():
    ...
    try:
        ...
        if first_word not in allowed_write and first_word not in ("SELECT", "WITH"):
            return jsonify(
                {"success": False, "error": f"Nieznane polecenie: {first_word}", "sql": sql_query}
            )
                    ...
                    try:
                        ...
                        if first_word == "UPDATE":
                            where_part = re.search(
                                r"\bWHERE\b(.+?)$", sql_query, re.IGNORECASE | re.DOTALL
                            )
                            table_match = re.search(
                                r"UPDATE\s+\[?(\w+)\]?", sql_query, re.IGNORECASE
                            )
                            ...
                            from_match = re.search(
                                r"\bFROM\b(.+)$", sql_query, re.IGNORECASE | re.DOTALL
                            )
                            if from_match:
        ...
        if not is_safe:
            return jsonify(
                {
                    "success": False,
                    "error": safe_err or "Niedozwolone zapytanie SQL",
                    "sql": sql_query,
                }
            )
    ...
    def generate():
        ...
        try:
            ...
            if not active_cols:
                ...
                _finish_heavy_task(
                    task_id, status="error", error=f"Brak kolumn do wektoryzacji w tabeli {table}"
                )
                task_id = ""
                return
            ...
            for row in _sql_fetch_table_rows(data_cur, table, active_cols, dialect, BATCH):
                ...
                _update_heavy_task(
                    task_id, done=done, total=total, progress_pct=pct, current_item=table
                )
...
def sql_vectorize_all():
    ...
    def generate():
        ...
        try:
            ok, task = _start_heavy_task(
                "sql_vectorize_all", "Wektoryzacja wszystkich tabel SQL", {}
            )
            if not ok:
                yield sse("error", _heavy_task_busy_payload(task))
                return
            ...
            yield sse(
                "start",
                {"start": True, "total_tables": len(tables), "tables": tables, "task_id": task_id},
            )
            ...
            for table_idx, table in enumerate(tables):
                try:
                    ...
                    if not active_cols:
                        yield sse(
                            "table_done",
                            {"table_done": True, "table": table, "rows": 0, "table_idx": table_idx},
                        )
                        continue
                    ...
                    for row in _sql_fetch_table_rows(data_cur, table, active_cols, dialect, BATCH):
                        ...
                        _update_heavy_task(
                            ...
                            total=len(tables),
                            progress_pct=round((table_idx + (pct / 100)) / len(tables) * 100)
                            if tables
                            else 100,
            ...
            _docs_cache["data"] = None
            _update_heavy_task(
                task_id, done=len(tables), total=len(tables), progress_pct=100, current_item=None
            )
            yield sse(
                "done",
                {
